...
function wpaal_add_affiliate_links($content) {  
		...
		if($ilcssclass) $illclass = $ilcssclass . " aalmanual";
		else $illclass = $lclass;
	...
	//	if($samekeyword > $samelink) $samekeyword = $samelink;  Removed on 28.06.2021, to let users add more links with the same keyword
		...
		$lcstyle = '';
		if(isset($linkcolor) && $linkcolor) $lcstyle = ' style="color:'. $linkcolor .';"';
		
		$ismulinks = get_option('aal_ismulinks');
		...
		if(is_feed()) {
			if('true' == get_option('aal_showrss') || true == get_option('aal_showrss')) {
			}
			else {
				return $content;
			}
		...
		if ( is_multisite() && !is_main_site() ) {
			if($ismulinks) {
				$myrows = array_merge($myrows,$wpdb->get_results( "SELECT id,link,keywords,meta FROM ". $wpdb->base_prefix ."automated_links WHERE stats <> 'disabled' OR stats IS NULL " ));
		...
		foreach($allkeys as $ident => $key) { { {  
					...
					if( ($samelinkvalue > $addedlinks[$link]) && $samekeyword > $addedkeys[$key] ) {
					  ...
					  if(is_string($content)) if(stripos($content, $key) !== false) {	
 						...
 						//if($key) if(!in_array('/'. $key .'/', $patterns)) { 
							...
							if(isset($meta->disclosureoff) && $meta->disclosureoff == 'off' ) {
								 $ildisclosureshow = '';
								 $disclosureshow = '';
							}
							else { 
								$ildisclosureshow = $ildisclosure;
								$disclosureshow = $disclosure;
							...
							if($samelinkvalue <= $samekeyword) 
								$locallimit[] = $samelinkvalue;
							else
								$locallimit[] = $samekeyword;
				...
				if(isset($regexp) && is_array($regexp)) { 
					...
					foreach($regexp as $regnumber => $reg1) {
						...
						if($locallimit[$regnumber] > $notimes - $sofar) $locallimit[$regnumber] = $notimes - $sofar;
						...
						if(is_string($content)) if(stripos($content, $keys2[$regnumber]) !== false) {
							...
							if($linkdistribution == "random" && strlen($content)<100000 ) {						
								...
								else $content = preg_replace($reg1, $replace[$regnumber], $content,$locallimit[$regnumber],$count);
								...
								if(is_string($content)) if(strlen($content)>100000) {
									...
									foreach($conar as $k => $con) {
										$conar[$k] = preg_replace($reg1, $replace[$regnumber], $conar[$k],$locallimit[$regnumber],$countar);
										$count = $count + $countar;
										if($countar>0) break;
									}
